...
# The certificate will be issued for the Common Name (CN) of `webhook-server.mix-scheduler-system.svc`, which is the
...
CN = webhook-server.mix-scheduler-system.svc
[ v3_req ]
...
[alt_names]
DNS.1 = webhook-server.mix-scheduler-system.svc
EOF
...
openssl req -new -key webhook-server-tls.key -subj "/CN=webhook-server.mix-scheduler-system.svc" -config server.conf \
